...
describe('Billing Route - Stripe Webhook', () => {
  it('should process checkout.session.completed and activate tenant', async () => {
    ...
    const bindMock = vi.fn().mockReturnValue({ run: runMock, first: vi.fn().mockResolvedValue({ email: 'admin@test.com' }) });
    ...
    const res = await billing.request(req, undefined, { ...mockEnv, Variables: { logger: mockLogger } } as any);
