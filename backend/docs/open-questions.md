...
- Implementar fallback com Gemini/Google GenAI direto (que já está disponível como dependência)
...
**Pergunta:** O projeto tem Google Gemini, Ollama (Gemma 4), Agent Hub, e o frontend tem `openaiService.ts` (agora `aiService.ts`). Quem é usado quando?
...
|------------|----------|-----------|
| `aiService.ts` (backend) | **Ollama** (Gemma 4) | `callGemma()` chama `${ollamaUrl}/api/chat` |
| `salesTools.ts` | **Agent Hub** | Chama `agent-hub.oconnector.tech` |
| `routes/ai.ts` (`/public-chat`) | **Ollama** (Gemma 4) | Usa `callGemma()` |
| `email-handler.ts` | Nenhum (parse apenas) | Usa PostalMime para extrair dados |

### Frontend (`aiService.ts`, antigo `openaiService.ts`)

| Função | Provider | Lógica |
|--------|----------|--------|
| `analyzePropertyImage` | **Ollama** (config.provider === 'ollama') | Chama Ollama local com modelo de visão |
| `generatePropertyDescription` | **Ollama** ou **Backend** (`/api/ai/generate`) | Depende da config do usuário |
| `summarizeConversation` | **Ollama** ou **Backend** | Depende da config |
| `suggestReply` | **Ollama** ou **Backend** | Depende da config |
| `generateMarketingCaption` | **Ollama** ou **Backend** | Depende da config |
| `analyzeClientProfile` | **Ollama** ou **Backend** | Depende da config |
| `fastAgentResponse` | **Ollama** ou **Backend** (`/api/ai/generate`) | Depende da config |
| `askGlobalAgent` | **Backend** (`/api/ai/public-chat`) | Sempre backend |
| `askMarketExpert` | **Ollama** ou **Backend** | Depende da config |
...
```typescript
{
  provider: 'ollama' | 'openai',  // 'ollama' é o default
...
- **Backend** usa Ollama (Gemma 4) para IA generativa e Agent Hub para skills de vendas
- **Frontend** pode usar Ollama local OU backend AI, dependendo da configuração do usuário
- **Google Gemini** está nas dependências (`@google/genai`, `@google/generative-ai`) mas **não é chamado em nenhum lugar do código analisado** — possivelmente é um fallback planejado mas não implementado
- **OpenAI** não é usado diretamente — o nome "openai" era legado, agora `aiService.ts` usa Ollama ou backend

**Recomendação:**
- Remover dependências `@google/genai` e `@google/generative-ai` se não forem usadas, OU implementar o provider Gemini
