...
export async function checkAndIncrementRateLimit(
  ...
  const existing = await db.prepare(
    `SELECT count FROM ai_rate_limits WHERE tenant_id = ? AND model = ? AND date = ?`
  ).bind(tenantId, model, today).first<{ count: number }>();
...
export async function getRateLimitStatus(
  ...
  `).bind(tenantId, today).all<{ model: string; count: number }>();
 ...
 */
export async function cleanupOldRateLimits(db: D1Database): Promise<number> {
  const weekAgo = new Date();
  weekAgo.setDate(weekAgo.getDate() - 7);
  const cutoffDate = weekAgo.toISOString().split('T')[0];

  const result = await db.prepare(
    `DELETE FROM ai_rate_limits WHERE date < ?`
  ).bind(cutoffDate).run();
