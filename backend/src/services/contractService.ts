...
export function getTemplate(type: 'sale' | 'rent'): string {
    if (type === 'sale') {
        return `
            ...
            <h3>OBJETO:</h3>
            <p>O imóvel situado em: {{property_address}}.</p>
            <h3>PREÇO:</h3>
            <p>O valor acordado é de <strong>R$ {{property_price}}</strong>.</p>
            <br>
            <p>Data: {{date}}</p>
            <br>
    ...
    return `
        ...
        <p>O imóvel situado em: {{property_address}}.</p>
        <h3>ALUGUEL:</h3>
        <p>O valor mensal é de <strong>R$ {{property_price}}</strong>.</p>
        <br>
        <p>Data: {{date}}</p>
    `;
}
...
export async function generateContractHtml(
    ...
    // 1. Fetch Data
    const tenant = await env.DB.prepare('SELECT name, owner_name, email FROM tenants WHERE id = ?').bind(tenantId).first<any>();
    const client = await env.DB.prepare('SELECT name, email, budget FROM clients WHERE id = ?').bind(clientId).first<any>();
    const property = await env.DB.prepare('SELECT title, location, price, listing_type FROM properties WHERE id = ?').bind(propertyId).first<any>();
