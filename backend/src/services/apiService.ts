...
export const apiService = {
  ...
  publishListing: async (env: Bindings, portal: string, tenantId: string, listingId: string) => {
    await env.DB.prepare('UPDATE properties SET publish_to_portals = 1, portal_url = ? WHERE id = ? AND tenant_id = ?')
  ...
  removeListing: async (env: Bindings, portal: string, tenantId: string, listingId: string) => {
    await env.DB.prepare('UPDATE properties SET publish_to_portals = 0, portal_url = NULL WHERE id = ? AND tenant_id = ?')
  ...
  syncListings: async (env: Bindings, portal: string, tenantId: string) => {
    ...
    const result = await env.DB.prepare('SELECT COUNT(*) as count FROM properties WHERE tenant_id = ? AND publish_to_portals = 1')
        .bind(tenantId)
        .first<{ count: number }>();
