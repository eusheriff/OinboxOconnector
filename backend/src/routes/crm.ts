...
crm.get('/clients', async (c) => {
    ...
    const { results } = await c.env.DB.prepare("SELECT * FROM clients WHERE tenant_id = ? ORDER BY created_at DESC")
        .bind(tenantId)
        .all();
...
crm.post('/clients', async (c) => {
    ...
    ).bind(id, tenantId, data.name, data.email, data.phone, data.status || 'Novo', data.budget, passwordHash)
        .run();
...
crm.post('/clients/:id/analyze', async (c) => {
    ...
    const clientData = await c.env.DB.prepare("SELECT * FROM clients WHERE id = ? AND tenant_id = ?").bind(clientId, tenantId).first();
    ...
    // 2. Construct Prompt
    const conversation = history.results.map((m: any) => `${m.role}: ${m.content}`).join("\n");
    const prompt = `
