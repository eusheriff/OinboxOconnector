...
export async function handleEmail(message: EmailMessage, env: Bindings, ctx: any) {
  const parser = new PostalMime();
  const email = await parser.parse(message.raw);

  console.log(`[Email Handler] Received email from: ${message.from} to: ${message.to}`);
  ...
  if (recipientUser.startsWith('leads.')) {
    ...
    console.log('[Email Handler] Invalid recipient format');
  ...
  // Identificar Origem
  if (message.from.includes('zap.com.br') || subject.toLowerCase().includes('zap')) source = 'Zap Imóveis';
  else if (message.from.includes('vivareal.com.br') || subject.toLowerCase().includes('viva')) source = 'VivaReal';
  else if (message.from.includes('olx.com.br') || subject.toLowerCase().includes('olx')) source = 'OLX';
  ...
  const emailMatch = content.match(/(?:Email|E-mail):\s*([a-zA-Z0-9._-]+@[a-zA-Z0-9._-]+\.[a-zA-Z0-9._-]+)/i);
  if (emailMatch) leadEmail = emailMatch[1].trim();
  else if (email.from) {
      // Se não achar no corpo, tenta pegar do sender se não for no-reply
      if (email.from && email.from.address && !email.from.address.includes('no-reply') && !email.from.address.includes('nao-responda')) {
          leadEmail = email.from.address;
      }
  }

  // Telefone: Padrões comuns
  const phoneMatch = content.match(/(?:Telefone|Tel|Celular|Whatsapp):\s*\(?(\d{2})\)?\s*(\d{4,5})-?(\d{4})/i);
  if (phoneMatch) {
  ...
  console.log(`[Email Handler] Extracted: ${leadName}, ${leadPhone}, ${source}`);

  if (!leadPhone && !leadEmail) {
      console.log('[Email Handler] No contact info found, skipping');
      return;
  }
  ...
  try {
      ...
      if (!tenant) {
          console.log(`[Email Handler] Tenant ${tenantId} not found`);
          return;
      }
      ...
      if (leadEmail) {
          existingClient = await env.DB.prepare("SELECT * FROM clients WHERE tenant_id = ? AND email = ?").bind(tenantId, leadEmail).first();
      }
      if (!existingClient && leadPhone) {
           existingClient = await env.DB.prepare("SELECT * FROM clients WHERE tenant_id = ? AND phone = ?").bind(tenantId, leadPhone).first();
      }
      ...
      if (existingClient) {
          ...
          console.log(`[Email Handler] Client exists: ${finalClientId}`);
          ...
          console.log(`[Email Handler] Created client: ${clientId}`);
      }
      ...
      console.error('[Email Handler] DB Error:', error);
  }
}
