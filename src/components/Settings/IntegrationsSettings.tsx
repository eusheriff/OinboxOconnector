...
const MASTER_INTEGRATIONS_LIBRARY: IntegrationItem[] = [
  ...
  },
  {
    ...
    type: 'messaging',
    icon: Instagram,
    description: 'Gerencie Directs e comentários.',
  ...
  },
  {
    ...
    type: 'messaging',
    icon: Facebook,
    description: 'Messenger da sua Fanpage.',
  ...
  },
  {
    ...
    type: 'messaging',
    icon: Mail,
    description: 'Conecte seu Gmail ou Outlook.',
  ...
  },
  {
    ...
    type: 'listing',
    icon: Building2,
    description: 'Sincronização automática de leads.',
    color: 'text-orange-600 bg-orange-50',
    authMethod: 'credentials',
    authFields: [
      { label: 'Login', type: 'text', placeholder: 'Usuário' },
  ...
  },
  {
    ...
    type: 'listing',
    icon: Globe,
    description: 'Gestão de anúncios classificados.',
...
const IntegrationsSettings: React.FC<IntegrationsSettingsProps> = ({ status, onStatusChange }) => {
  ...
  const [myIntegrations, setMyIntegrations] = useState<IntegrationItem[]>([
    {
      ...
      type: 'messaging',
      icon: MessageCircle,
      description: 'API Oficial (WABA) para automação e múltiplos atendentes.',
      color: 'text-green-600',
      authMethod: 'qr',
      connectedLabel: '5522992363462',
    },
  ]);
  ...
  // const [oauthStep, setOauthStep] = useState<'authorize' | 'select_pages' | 'complete'>('authorize');
  // const [qrProgress] = useState(0);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">
                  Necessário para analisar fotos de imóveis.
                </p>
              </div>
            </div>
          </div>
      ...
      <div className="mb-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {myIntegrations.map((item) => (
            <div
              key={item.id}
              className={`bg-white border rounded-xl p-5 transition-all relative group ${status[item.id] === 'connected' ? 'border-green-200 shadow-sm' : 'border-gray-200'}`}
            >
              <div className="flex justify-between items-start mb-4">
                ...
                {status[item.id] === 'connected' && (
                  <span className="text-[10px] font-bold bg-green-50 text-green-600 px-2 py-1 rounded-full border border-green-100 flex items-center gap-1">
              ...
              <button
                onClick={() => {
                  ...
                  if (item.authMethod === 'qr') setQrProgress(0);
                }}
          ...
          <button
            ...
            className="border-2 border-dashed border-gray-300 rounded-xl p-5 flex flex-col items-center justify-center gap-3 text-gray-400 hover:border-blue-500 hover:text-primary hover:bg-blue-50 transition-all min-h-[240px]"
          >
      ...
      <div className="max-w-7xl mx-auto">
        <div className="mb-8">
          ...
          <p className="text-gray-500">
            Gerencie conexões externas e preferências de Inteligência Artificial.
          </p>
        </div>
      ...
      {activeIntegration && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md animate-in zoom-in duration-200 relative">
            ...
            <div
              className={`w-12 h-12 rounded-xl mb-4 flex items-center justify-center ${activeIntegration.color}`}
            >
              <activeIntegration.icon className="w-6 h-6" />
            </div>
            <h3 className="text-xl font-bold text-foreground mb-2">
              Conectar {activeIntegration.name}
            </h3>
            ...
            {activeIntegration.authMethod === 'qr' && (
              <div className="bg-white p-4 border rounded-lg flex flex-col items-center justify-center mb-4">
                <img
                  src="https://api.qrserver.com/v1/create-qr-code/?size=150x150&data=Auth"
                  className="opacity-80 mb-2"
                />
                <p className="text-xs text-gray-400 text-center">
                  Escaneie para ativar a Agente Manú (+55 22...)
                </p>
              </div>
            )}
