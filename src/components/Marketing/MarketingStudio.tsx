...
const TEMPLATES: MarketingTemplate[] = [
  ...
  { id: 't4', name: 'Post - Luxo', format: 'post', label: 'ALTO PADR�O', color: 'bg-background' },
];
...
const MarketingStudio: React.FC = () => {
  ...
  useEffect(() => {
    const loadProperties = async () => {
      try {
        ...
        addToast('error', 'Falha ao carregar imóveis');
      } finally {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      addToast(
        'error',
        'Erro ao gerar imagem. Verifique se a imagem do imóvel permite CORS (Cloudflare R2 configurado).',
      );
    }
  };
