...
const CampaignManager: React.FC = () => {
  ...
  const runAiMatchmaker = (property: Property) => {
    ...
    setTimeout(() => {
      ...
      const matches = MOCK_CLIENTS.filter((_, idx) => idx % 2 === 0).slice(0, 5);
    ...
    <div className="animate-in fade-in slide-in-from-right duration-300">
      <div className="flex items-center gap-2 mb-6">
        <button
          ...
          className="text-sm text-gray-500 hover:text-foreground hover:underline"
        >
          ← Voltar
        </button>
          ...
          <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm min-h-[200px]">
            ...
            {!selectedProperty ? (
              <div className="flex flex-col items-center justify-center h-32 text-gray-400 text-center">
                ...
                <p className="text-sm">
                  Selecione um imóvel ao lado para a IA encontrar os compradores ideais.
                </p>
              </div>
            ) : isAnalyzing ? (
              <div className="flex flex-col items-center justify-center h-32 text-purple-600">
                ...
                <p className="text-xs text-gray-400">
                  Cruzando orçamento, localização e interesses.
                </p>
              </div>
            ) : (
              <div className="animate-in zoom-in duration-300">
                <div className="flex items-center justify-between mb-3">
                  <span className="text-sm font-bold text-slate-700">
                    Encontramos{' '}
                    <span className="text-green-600">{matchedClients.length} Leads Quentes</span>
                  </span>
                ...
                <div className="flex -space-x-2 mb-4 overflow-hidden">
                  {matchedClients.map((c) => (
                    <div
                      key={c.id}
                      className="w-10 h-10 rounded-full border-2 border-white bg-slate-200 flex items-center justify-center text-xs font-bold text-muted-foreground"
                      title={c.name}
                    >
                      {c.name.charAt(0)}
                    </div>
      ...
      <div className="flex justify-between items-center mb-8">
        <div>
          ...
          <p className="text-muted-foreground">
            Disparos individuais em massa para leads segmentados.
          </p>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          ...
          <p className="text-xs text-green-600 font-bold mt-1 flex items-center gap-1">
            <Target className="w-3 h-3" /> Alta precisão
          </p>
        </div>
        <div className="bg-white p-6 rounded-2xl border border-gray-200 shadow-sm">
          <h3 className="text-sm font-bold text-gray-400 uppercase tracking-wider mb-2">
          ...
          {activeCampaigns.map((camp) => (
            ...
            >
              <div className="flex items-center gap-4">
                <div
                  className={`p-3 rounded-full ${camp.type === 'whatsapp' ? 'bg-green-100 text-green-600' : 'bg-blue-100 text-primary'}`}
                >
                  {camp.type === 'whatsapp' ? (
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 flex items-center gap-1">
                    {camp.createdAt.toLocaleDateString()} • <Users className="w-3 h-3" />{' '}
                    {camp.targetAudienceCount} leads
                  </p>
                </div>
              </div>
