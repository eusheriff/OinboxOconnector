...
const PropertyList: React.FC<PropertyListProps> = ({
  ...
  const copyToClipboard = () => {
    ...
    addToast('success', 'Link do Feed XML copiado!');
  };
        ...
        <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4">
          <div className="bg-white rounded-xl shadow-2xl max-w-lg w-full p-6 animate-in fade-in zoom-in duration-200">
            ...
            <div className="bg-slate-50 p-3 rounded-lg border border-slate-200 flex items-center gap-2 mb-4">
              <code className="text-xs text-muted-foreground flex-1 break-all font-mono">
                {feedUrl}
              </code>
              <button
            ...
            <div className="bg-blue-50 text-blue-800 p-3 rounded-lg text-xs border border-blue-100">
              <strong>Dica:</strong> Envie este link para o gerente da sua conta no portal. A
              atualização dos imóveis será automática a cada 24h.
            </div>
              ...
              <div className="p-4 flex-1 flex flex-col">
                ...
                <div className="flex flex-wrap gap-1 mb-4">
                  {property.features.slice(0, 2).map((feat, i) => (
                    <span
                      key={i}
                      className="bg-gray-50 text-gray-600 px-2 py-1 rounded text-[10px] font-medium flex items-center gap-1"
                    >
