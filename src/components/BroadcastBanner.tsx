...
const BroadcastBanner: React.FC = () => {
    ...
    useEffect(() => {
        const fetchBroadcasts = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/platform/broadcasts`, {
    ...
    const getStyles = (type: string) => {
        switch (type) {
            case 'warning': return 'bg-yellow-500 text-black'; // Keep warning distinct
            case 'error': return 'bg-destructive text-destructive-foreground';
            case 'success': return 'bg-green-600 text-white'; // Keep success distinct
    ...
    return (
        <div className={`${getStyles(activeBroadcast.type)} px-4 py-3 shadow-md flex justify-between items-center z-40 relative`}>
