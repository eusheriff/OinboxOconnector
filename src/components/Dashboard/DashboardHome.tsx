...
const DashboardHome: React.FC<DashboardProps> = ({
  ...
  const hotLeads = clients.filter(
    (c) => c.temperature === 'Hot' || (c.leadScore && c.leadScore > 70),
  );
  ...
  const StatCard = ({ title, value, subtext, icon: Icon, color, trend }: StatCardProps) => (
    <div className="bg-card p-6 rounded-2xl border border-gray-200 shadow-sm hover:shadow-md transition-shadow">
      ...
      <div className="flex items-center gap-2 text-xs">
        <span
          className={`font-bold ${trend >= 0 ? 'text-green-600' : 'text-red-500'} flex items-center`}
        >
          {trend >= 0 ? (
        ...
        <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div>
            ...
            <p className="text-gray-500">
              Bem-vindo de volta! Aqui está o resumo da sua imobiliária hoje.
            </p>
          </div>
            ...
            <div className="p-4 space-y-3">
              {hotLeads.length > 0 ? (
                hotLeads.map((client) => (
                  <div
                    key={client.id}
                    className="flex items-center justify-between p-4 rounded-xl bg-card border border-gray-100 hover:border-red-200 hover:shadow-md transition-all group cursor-pointer"
                    onClick={() => onNavigate('MY_CLIENTS')}
                  >
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        <h4 className="font-bold text-foreground text-primary transition-colors">
                          {client.name}
                        </h4>
                        <p className="text-xs text-gray-500">
                          {client.notes || 'Interesse alto detectado'}
                        </p>
                      </div>
                    </div>
          ...
          <div className="space-y-6">
            <div className="bg-card p-6 rounded-2xl border border-gray-200 shadow-sm">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={() => onNavigate('LISTINGS_FORM')}
                  className="p-3 bg-blue-50 hover:bg-blue-100 rounded-xl text-blue-700 text-xs font-bold flex flex-col items-center gap-2 transition-colors"
                >
                  <Building2 className="w-5 h-5" /> Novo Imóvel
                </button>
                <button
                  onClick={() => onNavigate('MY_CLIENTS')}
                  className="p-3 bg-purple-50 hover:bg-purple-100 rounded-xl text-purple-700 text-xs font-bold flex flex-col items-center gap-2 transition-colors"
                >
                  <Users className="w-5 h-5" /> Cadastrar Cliente
                </button>
                <button
                  onClick={() => onNavigate('CRM')}
                  className="p-3 bg-green-50 hover:bg-green-100 rounded-xl text-green-700 text-xs font-bold flex flex-col items-center gap-2 transition-colors"
                >
                  <DollarSign className="w-5 h-5" /> Ver Pipeline
                </button>
                <button
                  onClick={() => onNavigate('AI_CONSULTANT')}
                  className="p-3 bg-indigo-50 hover:bg-indigo-100 rounded-xl text-indigo-700 text-xs font-bold flex flex-col items-center gap-2 transition-colors"
                >
