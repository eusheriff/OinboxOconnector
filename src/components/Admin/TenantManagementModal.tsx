...
interface TenantManagementModalProps {
    ...
    onManagePassword: (tenantId: string, action: 'create' | 'delete', password?: string) => Promise<void>;
    onImpersonate: (tenantId: string) => Promise<void>;
}

const TenantManagementModal: React.FC<TenantManagementModalProps> = ({ isOpen, onClose, tenant, onSave, onManagePassword, onImpersonate }) => {
    ...
    useEffect(() => {
        if (tenant) {
            setFormData({
                ...
                subscription_end: tenant.subscription_end ? new Date(tenant.subscription_end).toISOString().split('T')[0] : '',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white rounded-xl shadow-2xl w-full max-w-lg overflow-hidden animate-in zoom-in duration-200">
                <div className="bg-background px-6 py-4 flex justify-between items-center">
                    <h3 className="text-lg font-bold text-white">{isEditing ? `Gerenciar: ${tenant.name}` : 'Novo Inquilino'}</h3>
                ...
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Vencimento da Assinatura</label>
                        <input
                            type="date"
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-700 mb-1">Saldo / Bônus (R$)</label>
                            <input
                                type="number"
                                step="0.01"
                        ...
                        <div>
                            ...
                            <input
                                ...
                                onChange={(e) => setFormData({ ...formData, discount_plan: parseFloat(e.target.value) })}
                                className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary outline-none text-gray-900 bg-white"
                            />
                        </div>
                    </div>
                        ...
                        {isEditing && (
                            <div className="flex items-center gap-2 mb-4">
                                ...
                                <button
                                    type="button"
                                    onClick={() => {
                                        if (!newPassword) return alert('Digite uma senha');
