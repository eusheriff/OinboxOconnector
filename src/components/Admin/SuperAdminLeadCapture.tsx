...
// API Base URL - empty in dev (Vite proxy), absolute in prod
...
const SuperAdminLeadCapture: React.FC<SuperAdminLeadCaptureProps> = ({ onLogout }) => {
  ...
  const menuItems = [
    ...
    { view: AppView.SUPER_ADMIN_LEADS, icon: Users, label: 'Gerenciar Leads', badge: '245' },
    { view: AppView.SUPER_ADMIN_QUALIFICATION, icon: Star, label: 'Qualificação', badge: null },
    { view: AppView.SUPER_ADMIN_WHATSAPP_BOT, icon: MessageCircle, label: 'Bot WhatsApp', badge: null },
    { view: AppView.SUPER_ADMIN_CAMPAIGNS, icon: Megaphone, label: 'Campanhas', badge: '3' },
    { view: AppView.SUPER_ADMIN_INBOX, icon: Inbox, label: 'Inbox Omnichannel', badge: '12' },
    ...
    { view: AppView.SUPER_ADMIN_BUYER_LEADS, icon: Home, label: '🏠 Leads Compradores', badge: 'NEW' },
  ];
          ...
          {Object.entries(mockFunnelData).map(([stage, count], idx) => {
            ...
            return (
              <div key={stage} className="flex-1 flex flex-col items-center">
                ...
                />
                <span className="text-xs text-muted-foreground mt-2 text-center">{labels[idx]}</span>
              </div>
            );
          })}
        </div>
  ...
  const processLeads = async (placesToImport: any[]) => {
      ...
      try {
          ...
          const importRes = await fetch(`${API_BASE}/api/places/import`, {
              ...
              body: JSON.stringify({ places: placesToImport, searchQuery: `${searchQuery} in ${searchLocation}` })
          });
          ...
          // 2. Qualify
          setProcessingStatus('Qualifying leads...');
          // We need to fetch the newly imported leads IDs. Since we don't have them easily from import response (unless modified), 
          // we might just trigger qualify for all 'new' leads or similar. 
          // For MVP, we will assume backend handles this or we fetch recent leads.
          // Let's modify the flow to fetch recent leads first.
          
          // Simplified: Just listing them for now as user might want to review before AI
          setProcessingStatus('Auto-processing started... (See Leads tab)');
          setTimeout(() => setProcessingStatus(''), 3000);
    ...
    <div className="p-8 h-full flex flex-col">
      ...
      <div className="bg-card p-6 rounded-xl border border-border shadow-sm mb-6 flex-shrink-0">
        <div className="flex gap-4 mb-4">
          <div className="flex-1">
            <label className="block text-sm font-medium text-muted-foreground mb-1">Buscar por</label>
            <input
              type="text"
              value={searchQuery}
          ...
          <div className="w-32">
            <label className="block text-sm font-medium text-muted-foreground mb-1">Raio (km)</label>
            <input
              type="number"
              value={searchRadius}
      ...
      <div className="flex flex-1 gap-6 min-h-0">
        ...
        <div className="w-1/3 overflow-y-auto pr-2 space-y-4">
            {places.length > 0 ? (
            places.map((place) => (
                <div key={place.place_id} className="bg-card p-4 rounded-xl border border-border shadow-sm hover:shadow-md transition-shadow flex justify-between items-center group cursor-pointer"
                     onClick={() => {
                ...
                <div className="flex gap-4 items-start">
                    ...
                    {place.icon ? <img src={place.icon} className="w-6 h-6" alt="" /> : <MapPin className="w-6 h-6 text-muted-foreground" />}
                    </div>
                    <div>
                    <h3 className="font-bold text-foreground group-hover:text-primary transition-colors">{place.name}</h3>
  ...
  const renderSettings = () => {
    ...
    const testGeminiConnection = async () => {
        ...
        try {
            // Dynamically import to avoid top-level issues if package missing (though it is in package.json)
            const { GoogleGenerativeAI } = await import('@google/generative-ai');
            const genAI = new GoogleGenerativeAI(import.meta.env.VITE_GOOGLE_GEMINI_API_KEY || '');
            const model = genAI.getGenerativeModel({ model: "gemini-pro"});
            
            const result = await model.generateContent("Hello, are you online? Reply with 'Gemini Online ✅'");
                    ...
                    <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg border border-border">
                        <div>
                            ...
                            <div className="text-sm text-muted-foreground">
                                Key: {import.meta.env.VITE_GOOGLE_GEMINI_API_KEY ? 'Configurada •••••' : 'Não Configurada ❌'}
                            </div>
                            {geminiResponse && (
                                <div className={`mt-2 text-sm p-2 rounded ${geminiStatus === 'success' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'}`}>
                     ...
                     <div className="flex items-center justify-between p-4 bg-muted/50 rounded-lg border border-border">
                        <div>
                            <div className="font-semibold">Google Maps & Places</div>
                             <div className="text-sm text-muted-foreground">
                                Key: {import.meta.env.VITE_GOOGLE_PLACES_API_KEY ? 'Configurada •••••' : 'Não Configurada ❌'}
                            </div>
                        </div>
  ...
  const renderContent = () => {
    switch (activeView) {
      ...
      case AppView.SUPER_ADMIN_QUALIFICATION:
        return renderPlaceholder('Qualificação', '⭐', 'Configure critérios de scoring para leads');
      case AppView.SUPER_ADMIN_WHATSAPP_BOT:
        return renderPlaceholder('Bot WhatsApp', '💬', 'Configure a integração com Evolution API');
      case AppView.SUPER_ADMIN_CAMPAIGNS:
        return renderPlaceholder('Campanhas', '📢', 'Crie e gerencie campanhas de outreach');
      case AppView.SUPER_ADMIN_INBOX:
        return renderPlaceholder('Inbox Omnichannel', '📥', 'Gerencie todas as conversas em um só lugar');
...
const KanbanColumn = ({ id, title, leads, count }: { id: string, title: string, leads: any[], count: number }) => {
  const { setNodeRef } = useDroppable({ id });

  return (
    <div ref={setNodeRef} className="flex-1 min-w-[280px] bg-gray-50/50 rounded-xl border border-gray-100 flex flex-col h-full max-h-[calc(100vh-220px)]">
      <div className="p-3 border-b border-gray-100 flex justify-between items-center sticky top-0 bg-gray-50/95 backdrop-blur z-10 rounded-t-xl">
        <div className="flex items-center gap-2">
            ...
            <span className="bg-white border border-gray-200 px-2 py-0.5 rounded-full text-xs font-bold text-gray-500">{count}</span>
        </div>
...
const LeadsManager = () => {
    ...
    const fetchLeads = async () => {
        ...
        try {
            const res = await fetch(`${API_BASE}/api/leads?limit=100`); // Increase limit for Kanban
            const data: any = await res.json();
    ...
    const handleAiAction = async (leadId: string, action: 'analyze' | 'qualify') => {
        if (action === 'analyze') {
            const res = await fetch(`${API_BASE}/api/leads/${leadId}/analyze`, { method: 'POST' });
            const data: any = await res.json();
            if (data.success) {
    ...
    <div className="p-8 h-full flex flex-col">
      <div className="flex justify-between items-center mb-6 shrink-0">
        <div>
           ...
           <p className="text-muted-foreground text-sm">Visualize e gerencie seu pipeline de vendas</p>
        </div>
        ...
        <div className="flex gap-2">
          ...
          <button onClick={fetchLeads} className="p-2 border rounded-lg hover:bg-gray-50 bg-white"><Activity className="w-4 h-4" /></button>
      ...
      {viewMode === 'kanban' ? (
        <DndContext collisionDetection={closestCenter} onDragEnd={handleDragEnd}>
            ...
            <DragOverlay>
                {/* Optional: Add drag overlay customization here */}
            </DragOverlay>
        </DndContext>
      ) : (
        ...
        <table className="w-full text-left">
          ...
          <tbody className="divide-y divide-gray-100">
            {isLoadingLeads ? <tr><td colSpan={6} className="p-4 text-center">Carregando...</td></tr> : 
            leads.map((lead, idx) => (
              <tr key={lead.id || idx} className="hover:bg-gray-50 cursor-pointer">
                ...
                <td className="p-4">
                  <span className={`px-2 py-1 text-xs font-bold rounded-md ${
                    ...
                    {lead.status === 'responded' ? 'Respondeu' :
                ...
                <td className="p-4 text-right">
                  <div className="flex gap-1 justify-end">
                    <button onClick={() => handleAiAction(lead.id, 'analyze')} className="p-1 hover:bg-purple-100 rounded text-purple-600" title="Gerar Pitch IA">
...
const BuyerLeadsAdmin = () => {
    ...
    const [formData, setFormData] = useState({
      name: '', phone: '', email: '', interest_type: 'compra',
      property_type: 'casa', city: '', neighborhood: '',
      budget_min: '', budget_max: '', bedrooms: '', notes: '',
    });

    const mockBuyerLeads = [
      { id: '1', name: 'João Silva', phone: '(11) 99999-1234', interest_type: 'compra', property_type: 'casa', city: 'São Paulo', budget_max: 500000, ai_score: 85, status: 'available' },
      { id: '2', name: 'Maria Santos', phone: '(11) 98888-5678', interest_type: 'aluguel', property_type: 'apartamento', city: 'São Paulo', budget_max: 3000, ai_score: 72, status: 'available' },
    ];

    return (
      <div className="p-8">
        <div className="flex justify-between items-center mb-6">
          <div><h1 className="text-2xl font-bold text-foreground">🏠 Leads de Compradores</h1></div>
