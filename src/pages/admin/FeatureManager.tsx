...
const FeatureManager: React.FC = () => {
    ...
    const [newFlag, setNewFlag] = useState({ key: '', description: '', is_enabled: false, rules: '{}' });
    const [isCreating, setIsCreating] = useState(false);

    const fetchFlags = async () => {
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/feature-flags`, {
    ...
    const handleSave = async (flag: any) => {
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/feature-flags`, {
                method: 'POST',
                headers: {
            ...
            alert('Erro ao salvar feature flag.');
        }
    };
    ...
    const handleDelete = async (key: string) => {
        ...
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/feature-flags/${key}`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${localStorage.getItem('oconnector_token')}` }
            });
            fetchFlags();
            ...
            {isCreating && (
                <div className="bg-card p-6 rounded-xl border border-border mb-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        <input
                            type="text" placeholder="Chave (ex: new_dashboard)"
                    ...
                    <div className="flex justify-end gap-2">
                        ...
                        <button onClick={() => handleSave(newFlag)} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded flex items-center gap-2">
                            <Save size={18} /> Salvar
                        </button>
                    </div>
                </div>
                    ...
                    <tbody className="divide-y divide-slate-700">
                        {flags.map((flag) => (
                            <tr key={flag.key} className="hover:bg-slate-750 transition-colors">
                                ...
                                <td className="p-4">
                                    <span className={`px-2 py-1 rounded text-xs font-bold ${flag.is_enabled ? 'bg-green-900 text-green-200' : 'bg-gray-700 text-gray-300'}`}>
                                        {flag.is_enabled ? 'Ativo' : 'Inativo'}
                                    </span>
                                </td>
                                <td className="p-4 flex gap-2">
                                    <button onClick={() => {
                                        setNewFlag({ ...flag, rules: typeof flag.rules === 'string' ? flag.rules : JSON.stringify(flag.rules) });
                                        setIsCreating(true);
                                    }} className="text-blue-400 hover:text-blue-300">Editar</button>
                                    <button onClick={() => handleDelete(flag.key)} className="text-red-400 hover:text-red-300">
                                        <Trash2 size={18} />
                                    </button>
                                </td>
                            </tr>
