...
const AIControlPanel: React.FC = () => {
    ...
    useEffect(() => {
        const fetchConfig = async () => {
            try {
                const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/ai-config`, {
    ...
    const handleSave = async () => {
        setSaving(true);
        try {
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/ai-config`, {
                method: 'POST',
                headers: {
        ...
        <div className="space-y-8">
            ...
            <div className="bg-card p-6 rounded-xl border border-border shadow-lg space-y-4">
                ...
                <p className="text-gray-400 text-sm">
                    Este prompt define a personalidade base e as regras de negócio para todas as IAs dos inquilinos.
                    O contexto específico de cada imobiliária será injetado automaticamente.
                </p>
                <textarea
