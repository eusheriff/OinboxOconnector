...
const EnterpriseBuyerLeads: React.FC<EnterpriseBuyerLeadsProps> = ({
    ...
    const updatedLeads = leads.map((l) =>
      l.id === lead.id
        ? {
            ...l,
            accessed: true,
            phone: '(11) 99999-' + Math.floor(1000 + Math.random() * 9000),
  ...
  const handleMarkContacted = (leadId: string) => {
    setLeads(leads.map((l) => (l.id === leadId ? { ...l, contacted: true } : l)));
            ...
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-4">
                  ...
                  >
                    {lead.property_type === 'casa' && (
                      <Home
                        className={`w-5 h-5 ${lead.interest_type === 'compra' ? 'text-blue-600' : 'text-orange-600'}`}
                      />
                    )}
                    {lead.property_type === 'apartamento' && (
                      <Building2
                        className={`w-5 h-5 ${lead.interest_type === 'compra' ? 'text-blue-600' : 'text-orange-600'}`}
                      />
                    )}
                    {lead.property_type === 'terreno' && (
                      <MapPin
                        className={`w-5 h-5 ${lead.interest_type === 'compra' ? 'text-blue-600' : 'text-orange-600'}`}
                      />
                    )}
                    {lead.property_type === 'comercial' && (
                      <Building2
                        className={`w-5 h-5 ${lead.interest_type === 'compra' ? 'text-blue-600' : 'text-orange-600'}`}
                      />
                    )}
                  ...
                  <div>
                    <div className="flex items-center gap-2">
                      ...
                      <span
                        className={`px-2 py-0.5 text-xs font-bold rounded-full ${getScoreColor(lead.ai_score)}`}
                      >
                        {lead.ai_score}
                      </span>
                      {lead.contacted && (
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="text-right">
                    ...
                    <p className="font-bold text-gray-900">
                      {formatBudget(lead.budget_min, lead.interest_type)} -{' '}
                      {formatBudget(lead.budget_max, lead.interest_type)}
                    </p>
                  </div>
