...
const SubscriptionManager: React.FC = () => {
  ...
  const handleManageSubscription = async () => {
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/billing/portal`,
        {
          method: 'POST',
          headers: {
      ...
      if (data.url) {
        ...
        addToast('error', 'Erro ao abrir portal: ' + (data.error || 'Unknown error'));
      }
    } catch (error) {
      addToast('error', 'Erro ao conectar com Stripe');
    }
  };
