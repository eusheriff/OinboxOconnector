...
const SystemHealth: React.FC = () => {
    ...
    const fetchHealth = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/health-check`, {
        ...
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                ...
                <div className="bg-card p-6 rounded-xl border border-border flex flex-col items-center justify-center text-center">
                    ...
                    <h3 className="text-xl font-bold text-white mb-1">Sistema {health.status === 'healthy' ? 'Operacional' : 'Instável'}</h3>
                    <p className="text-gray-400 text-sm">Última atualização: {new Date(health.timestamp).toLocaleTimeString()}</p>
                </div>
