...
const AIAnalytics: React.FC = () => {
    ...
    const fetchData = async () => {
        setLoading(true);
        try {
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/admin/ai-usage`, {
                    ...
                    {data.map((item, index) => (
                        <div key={index} className="space-y-1">
                            <div className="flex justify-between text-sm text-gray-300">
                                <span className="font-bold">{item.tenant_name} <span className="text-gray-500 font-normal">({item.provider}/{item.model})</span></span>
                            ...
                            <div className="text-xs text-gray-500 text-right">{item.request_count} requisições</div>
                        </div>
                    ))}
