...
const WhatsAppManager: React.FC = () => {
    ...
    const fetchStatus = async () => {
        ...
        try {
            ...
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/whatsapp/status`, {
    ...
    const fetchQrCode = async () => {
        try {
            const token = localStorage.getItem('oconnector_token');
            const response = await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/whatsapp/qrcode`, {
                headers: { 'Authorization': `Bearer ${token}` }
            });
    ...
    const handleLogout = async () => {
        ...
        try {
            const token = localStorage.getItem('oconnector_token');
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/whatsapp/logout`, {
                method: 'DELETE',
                headers: { 'Authorization': `Bearer ${token}` }
            });
            fetchStatus();
    ...
    const handleReconnect = async () => {
        try {
            const token = localStorage.getItem('oconnector_token');
            await fetch(`${import.meta.env.VITE_API_URL || 'http://localhost:8787'}/api/whatsapp/reconnect`, {
                method: 'POST',
                headers: { 'Authorization': `Bearer ${token}` }
            });
                    ...
                    <div className="flex flex-col items-center justify-center py-8 space-y-4">
                        {connectionStatus === 'connected' ? (
                            <>
                                ...
                                <p className="text-green-400 text-sm bg-green-400/10 px-3 py-1 rounded-full">Evolution API v2</p>
                            </>
                        ) : (
                            <>
                                ...
                                <p className="text-gray-400 text-center mb-4">Escaneie o QR Code abaixo para conectar sua conta WhatsApp Business.</p>
                ...
                <div className="bg-card p-6 rounded-xl border border-border shadow-lg">
                    ...
                    <div className="space-y-4 text-gray-300">
                        ...
                        <div className="mt-6 p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
                            ...
                            <p className="text-sm">
                                Mantenha seu celular conectado à internet para garantir o funcionamento do bot.
                                Esta integração utiliza a Evolution API Oficial.
                            </p>
                        </div>
