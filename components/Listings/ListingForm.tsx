...
const ListingForm: React.FC = () => {
  ...
  const platforms = [
    ...
    { id: Platform.FACEBOOK, label: 'Facebook', icon: Facebook, color: 'bg-blue-50 text-primary border-blue-200' },
          ...
          <div className="lg:col-span-5 space-y-6 order-2 lg:order-1">
            ...
            <div className="bg-gray-50 p-5 rounded-xl border border-gray-200">
              <div className="flex justify-between items-center mb-4">
                ...
                <button type="button" onClick={handleSelectAllPlatforms} className="text-xs text-primary font-medium hover:underline">
                  {selectedPlatforms.length === platforms.length ? 'Desmarcar' : 'Selecionar Todos'}
                </button>
              </div>
              <div className="space-y-2 max-h-[300px] overflow-y-auto pr-1">
                {platforms.map((p) => (
                  <label key={p.id} className={`flex items-center p-3 rounded-lg border cursor-pointer transition-all ${selectedPlatforms.includes(p.id) ? 'bg-white border-blue-500 shadow-sm ring-1 ring-primary' : 'bg-white border-gray-200 hover:border-blue-300'}`}>
                    <input
                      type="checkbox"
          ...
          <div className="lg:col-span-7 space-y-6 order-1 lg:order-2">
            ...
            <div>
              ...
              <div className="flex gap-4">
                <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${formData.listingType === 'sale' ? 'bg-blue-50 border-blue-500 text-blue-700 shadow-sm' : 'bg-white border-gray-200 text-muted-foreground hover:border-blue-300'}`}>
                  <input
                    type="radio"
                    name="listingType"
                    className="hidden"
                ...
                <label className={`flex-1 flex items-center justify-center gap-2 p-3 rounded-lg border cursor-pointer transition-all ${formData.listingType === 'rent' ? 'bg-purple-50 border-purple-500 text-purple-700 shadow-sm' : 'bg-white border-gray-200 text-muted-foreground hover:border-purple-300'}`}>
                  <input
                    type="radio"
                    name="listingType"
                    className="hidden"
            ...
            <div className="flex gap-4">
              <button type="button" className="flex-1 py-3 border border-gray-300 rounded-lg text-muted-foreground font-medium hover:bg-gray-50 transition-colors">
                Salvar Rascunho
              </button>
              <button
                type="submit"
