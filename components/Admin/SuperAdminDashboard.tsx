                ...
                <div className="space-y-4">
                    <div className="flex items-center justify-between">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm font-medium text-slate-700">Cloudflare Workers (Edge)</span>
                        </div>
                ...
                <tbody className="divide-y divide-gray-100">
                    {tenants.map(tenant => (
                        <tr key={tenant.id} className="hover:bg-gray-50">
                            ...
                            <td className="p-4">
                                {tenant.status === 'Active' && <span className="flex items-center gap-1 text-xs font-bold text-green-600"><CheckCircle2 className="w-3 h-3"/> Ativo</span>}
                                {tenant.status === 'Overdue' && <span className="flex items-center gap-1 text-xs font-bold text-red-600"><AlertCircle className="w-3 h-3"/> Atrasado</span>}
                                {tenant.status === 'Trial' && <span className="flex items-center gap-1 text-xs font-bold text-orange-600"><Activity className="w-3 h-3"/> Trial</span>}
                            </td>
                            <td className="p-4 text-sm font-mono text-slate-700">
                                R$ {tenant.mrr.toLocaleString()}
                            </td>
                            <td className="p-4 text-xs text-gray-500">
                                {tenant.joinedAt.toLocaleDateString()}
                            </td>
