    ...
    <div className="flex-1 bg-gray-50 p-4 md:p-8 overflow-y-auto h-full">
      <div className="max-w-4xl mx-auto">
        ...
        <div className="mb-8">
            ...
            <p className="text-muted-foreground">Crie minutas jurídicas personalizadas em segundos. Selecione as partes e a IA cuida do resto.</p>
        </div>
        ...
        <div className="flex items-center justify-between mb-8 px-4">
            {[1, 2, 3].map(s => (
                <div key={s} className="flex items-center gap-2">
                    ...
                    <span className={`text-sm font-medium ${step >= s ? 'text-foreground' : 'text-gray-400'}`}>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden min-h-[400px]">
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                {MOCK_CLIENTS.map(client => (
                                    ...
                                    >
                                        ...
                                        {selectedClient?.id === client.id && <CheckCircle2 className="w-5 h-5 text-primary ml-auto" />}
                                    </div>
                                ))}
                            </div>
                        </div>
                            ...
                            <div className="space-y-2 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
                                {MOCK_PROPERTIES.map(prop => (
                                    ...
                                    >
                                        ...
                                        {selectedProperty?.id === prop.id && <CheckCircle2 className="w-5 h-5 text-primary ml-auto" />}
                                    </div>
                                ))}
                            </div>
                        </div>
                    ...
                    <div className="mt-8 pt-6 border-t border-gray-100 flex justify-between items-center">
                        <div className="flex gap-4">
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="type" className="w-4 h-4 text-primary" checked={contractType === 'sale'} onChange={() => setContractType('sale')} />
                                <span className="text-sm font-medium text-slate-700">Compra e Venda</span>
                            </label>
                            <label className="flex items-center gap-2 cursor-pointer">
                                <input type="radio" name="type" className="w-4 h-4 text-primary" checked={contractType === 'rent'} onChange={() => setContractType('rent')} />
            ...
            {step === 2 && generated && (
                 <div className="absolute inset-0 bg-white z-10 flex flex-col items-center justify-center p-8 animate-in zoom-in duration-300">
                    ...
                    <h2 className="text-2xl font-bold text-foreground mb-2">Contrato Gerado com Sucesso!</h2>
