...
import { Phone, Mail, MapPin, ExternalLink, User, Calendar, BrainCircuit, Loader2, Tag, FileText, Upload, Check, AlertCircle, Eye, Mic, StopCircle } from 'lucide-react';
...
const ClientDetails: React.FC<ClientDetailsProps> = ({ conversation }) => {
  ...
  const [voiceNoteResult, setVoiceNoteResult] = useState<any>(null);
  ...
  const handleVoiceNoteClick = async () => {
      if (isRecordingVoiceNote) {
          ...
          const simulatedTranscription = "O cliente visitou o imóvel hoje e gostou muito da área de lazer, mas achou o preço um pouco alto. Ele disse que o orçamento máximo é 900 mil. Fiquei de enviar outras opções no centro.";
          
          setTimeout(async () => {
              const insights = await processVoiceNote(simulatedTranscription);
              setVoiceNoteResult(insights);
              setVoiceProcessing(false);
              // Update Profile locally with insights
              if (insights && insights.budgetUpdate && profile) {
