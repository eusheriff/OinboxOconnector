...
import { Search, Plus, MoreVertical, Phone, Mail, User, X, Flame, Snowflake, ThermometerSun, BrainCircuit, Loader2 } from 'lucide-react';
            ...
            <tbody className="divide-y divide-gray-100">
              {filteredClients.map((client) => (
                <tr key={client.id} className="hover:bg-blue-50/50 transition-colors group">
                  ...
                  <td className="p-4">
                    <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${client.status === 'Novo' ? 'bg-blue-100 text-blue-800' :
                    ...
                    <div className="flex items-center gap-2">
                      <div className="w-16 h-1.5 bg-gray-100 rounded-full overflow-hidden">
                        <div
                          ...
                          className={`h-full ${(client.leadScore || 0) > 70 ? 'bg-red-500' :
                            (client.leadScore || 0) > 40 ? 'bg-orange-400' : 'bg-blue-400'
                            }`}
                        ></div>
                      </div>
                      <span className="text-xs font-bold text-muted-foreground">{client.leadScore || 0}</span>
                    </div>
                    {client.aiSummary && (
                      <p className="text-[10px] text-gray-500 mt-1 truncate max-w-[100px]" title={client.aiSummary}>
                        {client.aiSummary}
                      </p>
                    )}
                  </td>
                  ...
                  <td className="p-4 text-right flex items-center justify-end gap-2">
                    ...
                    >
                      {analyzingId === client.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <BrainCircuit className="w-4 h-4" />}
                    </button>
