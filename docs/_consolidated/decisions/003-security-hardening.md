...
## Decisão
1. **Placeholder Obliteration**: Todas as senhas reais ou genéricas (ex: "password123") e chaves de API em arquivos de teste e mocks DEVEM ser substituídas por placeholders descritivos (ex: `test-pass-000`, `mock-api-key-val`).
...
3. **Variable Obfuscation**: Variáveis locais que armazenam chaves (mesmo quando originadas com segurança do `env`) não devem usar nomes em CAIXA ALTA (como `GOOGLE_API_KEY`) para evitar gatilhos em scanners de segurança básicos, optando por camelCase (ex: `googleApiKey`).
...
- **Positivas**: 
    ...
    - Maior clareza sobre o que é dado real vs dado de teste.
- **Negativas**: 
    - Exige atualização manual dos testes se os placeholders mudarem drasticamente.
    - Necessidade de renomear variáveis locais em prol de conformidade com ferramentas de terceiros.
