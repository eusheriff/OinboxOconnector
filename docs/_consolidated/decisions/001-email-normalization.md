...
**Status**: Aceito
**Contexto**: O sistema utilizava emails como identificadores de login sensíveis a mAutomationúsculas no banco D1 (SQLite), causando falhas frequentes de autenticação (401). Além disso, não havia diferenciação clara de portal para clientes externos.
...
- **Negativas**: Necessidade de migração/update caso existam emails legados em mAutomationúsculas que precisem ser preservados (não é o caso atual).
