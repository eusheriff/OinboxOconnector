...
O log de depuração mostrou que o roteador estava casando a rota `/app/whatsapp` com a rota genérica `/*` definida para as páginas públicas antes de verificar as rotas internas do pAutomationnel do cliente. Além disso, o componente de WhatsApp possuía erros de importação que poderiam causar falhas de renderização.
...
- **Negativas**: Pequeno aumento na verbosidade do `App.tsx` (mAutomations linhas de `<Route>`).
