...
1. **Tipos duplicados em 3 locais**: `src/types.ts` (278 linhas), `backend/src/types.ts` (66 linhas), `shared/types/index.ts` (204 linhas) — sem uso consistente do `shared/`.
...
5. **Documentação dispersa**: Apenas 7 arquivos `.md` no projeto, sendo 5 dentro de `docs/_consolidated` e 1 na raiz. Faltam ADRs formais.
...
8. **Migrações SQL avulsas na raiz**: `backend/migration_lead_ops.sql` e `backend/migration_lead_ops_part2.sql` fora da pasta `migrations/`.
...
- Testes locais via Docker não refletem comportamento real
...
| `backend/migration_lead_ops.sql`       | Fora de lógica       | Na raiz de `/backend/` ao invés de `/migrations/` | Baixo | Mover para `migrations/`             | Não          |
| `backend/migration_lead_ops_part2.sql` | Fora de lógica       | Idem                                              | Baixo | Mover para `migrations/`             | Não          |
...
| `evolution-api-fly/`                   | Candidato a obsoleto | Apenas 1 arquivo filho, sem referência no código  | Baixo | Verificar propósito                  | Sim          |
| `data/` directory                      | Sobra                | Diretório na raiz com 1 arquivo                   | Baixo | Avaliar necessidade                  | Sim          |
| `docs/EVOLUTION_API_EASYPANEL.md`      | Fora de lógica       | Fora de `_consolidated/`                          | Baixo | Mover para `02_runbooks/`            | Não          |
...
4. **Resolver ESLint disables críticos**:
   - `backend/src/types.ts` - tipar explicitamente o binding AI
   - `backend/src/middleware/auth.ts` - adicionar tipos
   - `backend/src/routes/leads.ts` - adicionar tipos

### L2 - Mudanças Estruturais (1-2 semanas)
...
```
docs/_consolidated/
├── README.md            ← Índice central (já existe)
...
> **CAUTION**: Os itens abaixo requerem CONFIRMAÇÃO EXPLÍCITA antes de qualquer ação.
...
| `data/`              | Diretório na raiz                       | Verificar se é usado pelo Docker ou dev local |
...
> **IMPORTANTE**: Solicito **CONFIRMAÇÃO** para prosseguir com as ações L0 listadas acima.
