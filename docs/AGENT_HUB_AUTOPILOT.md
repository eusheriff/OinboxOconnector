...
O **Agent Hub** é um serviço externo de orquestração de IA que recebe requests do Oconnector backend e retorna respostas processadas por modelos de linguagem.
...
O Agent Hub é chamado em dois momentos principAutomations:
...
- Quem mantém o Agent Hub? � do mesmo time ou time separado?
...
| `backend/src/services/autopilot/`             | DemAutomations módulos do autopilot |

### 2.5 O que o Autopilot faz

_(Baseado no nome e contexto � ler o código do scheduler.ts para detalhes exatos)_
